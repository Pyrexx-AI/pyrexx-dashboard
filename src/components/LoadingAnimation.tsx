...
/*
  ...
  ─────────────────────────────────────────────────────────────────
  Uses the actual Pyrexx logo mark SVG paths (paths 1-10 from
  public/pyrexx-mask.svg, text paths 11-21 excluded).

  ANIMATION:
  A linearGradient (teal ↔ purple) is applied to the two colored
  ribbon paths (purple #3 and teal #4). An <animateTransform>
  rotates the gradient around the logo's visual center (835, 674),
  making the colors flow clockwise — teal flows into purple's
  position and purple flows into teal's — exactly as requested.

  The logo shape, dark border, inner white gap, and white interlocking
  separators remain fully static. Only the colors move.

  REDUCED MOTION:
  animateTransform is conditionally rendered. Without it, the
  gradient sits at 0° — teal left, purple right — matching the
  actual static logo.
*/
...
export default function LoadingAnimation() {
  ...
  // Visual center of the logo mark in SVG root coordinates (0 0 1667 1655)
  const CX = 835;
  const CY = 674;
  ...
  return (
    ...
    >
      {/* Ambient glow — suppressed for prefers-reduced-motion */}
      {!reducedMotion && (
        <motion.div
          ...
          style={{
            ...
            height: 320,
            background:
              "radial-gradient(circle, rgba(72,196,198,0.16) 0%, rgba(137,82,165,0.10) 55%, transparent 100%)",
          }}
      ...
      {/*
        Logo badge — cream background container so the logo renders
        exactly as designed (dark border visible against light ground).
      */}
      <motion.div
        ...
        style={{
          ...
          background: "#fafaf7",
          boxShadow:
            "0 0 0 1.5px rgba(72,196,198,0.25), 0 24px 64px rgba(0,0,0,0.45), 0 4px 16px rgba(137,82,165,0.2)",
        }}
        initial={
      ...
      >
        <svg
          ...
          /*
            viewBox crops to just the logo mark area.
            Original is 0 0 1667 1655; text starts near y=1225.
            Cropped to the double-X silhouette only.
          */
        ...
        >
          <defs>
            {/*
              THE COLOR-FLOW GRADIENT
              ──────────────────────────────────────────────────────
              Horizontal gradient: teal (left) → purple (right).
              gradientUnits="userSpaceOnUse" so coordinates are in
              the SVG root space, consistent across all paths.

              <animateTransform> rotates the gradient 360° around
              the logo's visual center (CX, CY). As it spins:
               • 0°   → teal-left / purple-right  (matches the logo)
               • 90°  → teal-top  / purple-bottom
               • 180° → teal-right / purple-left  (swapped)
               • 270° → teal-bottom / purple-top
               • 360° → back to start

              The wide flat zones (0–40% and 60–100%) keep each color
              distinct rather than a soft blend, making the "two solid
              colors chasing each other clockwise" effect clear.
            */}
            <linearGradient
              id="pyrexx-flow"
            ...
            >
              ...
              <stop offset="40%"  stopColor="#48C4C6" />
              <stop offset="60%"  stopColor="#8952A5" />
              <stop offset="100%" stopColor="#8952A5" />

              {/* Conditionally animate — omit entirely for reduced-motion */}
              {!reducedMotion && (
                <animateTransform
          ...
          {/* ── PATH [1]: Dark outer silhouette / border ────────── */}
          <path
            fill="#414140"
          ...
          {/* ── PATH [2]: White inner gap (sits inside the border) ── */}
          <path
            fill="#fafaf7"
          ...
          {/*
            ── PATHS [3] + [4]: ANIMATED COLOR RIBBONS ─────────────
            Both paths get fill="url(#pyrexx-flow)".
            Path [3] was purple (#7d56a1) — the right-X ribbon.
            Path [4] was teal (#60c3c9)   — the left-X ribbon.
            Sharing the same gradient instance means both ribbons
            show matching colors at the same gradient position,
            creating a unified flow across the whole logo mark.
          */}

          {/* PATH [3] — right-X ribbon (was purple) */}
          <path
          ...
          {/*
            ── PATH [8]: Top-center bridge — animated ────────────────
            Original fill was url(#a) — a cyan→teal gradient.
            Has transform="scale(1.2648 1.26432)" applied to the path.
            We apply the same pyrexx-flow gradient so the colors
            stay visually consistent with the rest of the animation.
            Because gradientUnits="userSpaceOnUse" coordinates are in
            root space (not scaled space), the color reads slightly
            differently here — but at this small size in the top-center
            it reads as the correct mid-transition hue.
          */}
          <path
            fill="url(#pyrexx-flow)"
          ...
          {/*
            ── PATHS [5,6,7,9,10]: White interlocking separators ─────
            These are the critical white cutout shapes that create
            the "ribbon goes over / under" interlocking effect.
            They remain fully static — only the colors move.
          */}

          {/* [5] Lower-left cutout */}
          <path
            fill="#fafaf7"
            d="M686.11 768.27c7.46 9.393 15.831 21.657 22.927 31.524l40.25 56.209c6.766 9.545 19.04 28.02 26.233 35.953-5.863 8.279-13.437 18.406-18.75 26.816-14.239 16.662-24.704 37.6-40.015 53.763-28.998 30.61-68.327 25.064-106.661 25.03l-88.078.09 106.109-149.079 35.554-50.014c6.737-9.427 15.109-21.713 22.43-30.293"
          />
          {/* [6] Upper-left cutout */}
          <path
            fill="#fafaf7"
            d="M526.073 373.738c33.701.805 67.77-.083 101.329.273 34.111.363 65.419-5.597 90.744 22.698 11.335 12.666 19.833 27.177 30.034 40.712 6.612 9.457 22.546 29.34 27.658 38.523-8.99 10.518-22.49 30.377-30.819 41.966l-58.617 81.704c-51.907-71.05-101.548-144.083-153.38-215.211-1.956-2.685-5.933-7.747-6.949-10.665"
          />
          {/* [7] Center-crossing cutout */}
          <path
            fill="#fafaf7"
            d="M836.28 560.32a7490 7490 0 0 0 58.179 81.616c7.068 9.884 23.99 32.406 29.348 41.995-8.235 13.362-22.015 31.42-31.596 44.748l-56.846 79.001-87.677-123.46c4.831-8.819 23.963-34.552 30.495-43.507 19.142-26.236 38.419-54.875 58.098-80.393"
          />
          {/* [9] Upper-right cutout */}
          <path
            fill="#fafaf7"
            d="M896.418 475.575c12.203-15.717 23.797-33.341 35.44-49.508 6.429-8.926 13.344-19.865 20.722-27.874a81.5 81.5 0 0 1 18.733-15.215 62.9 62.9 0 0 1 23.766-8.271c10.803-1.389 143.615-2.314 150.84-.414L1042.332 519.77c-17.793 24.907-38.49 55.676-56.89 79.614-11.999-15.396-24.03-33.023-35.446-49.02z"
          />
          {/* [10] Lower-right cutout */}
          <path
            fill="#fafaf7"
            d="M985.449 768.603c4.013 3 25.026 33.64 29.487 40.018 44.25 63.267 90.196 125.546 134.27 188.906l-90.437-.011c-25.78.083-53.6 2.858-77.659-7.297-24.281-10.248-35.348-29.367-50.049-49.705-11.578-16.017-23.089-32.792-35.279-48.35 8.029-9.198 17.793-24.536 25.462-34.7 21.45-28.427 42.248-61.118 64.205-88.861"
          />
        </svg>
      </motion.div>

      {/* "Pyrexx AI" wordmark below the badge */}
      <motion.p
        className="relative z-10 mt-6 text-[11px] font-semibold tracking-[0.22em] uppercase"
