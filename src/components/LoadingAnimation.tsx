...
/*
  ...
  1. ACCURATE RECREATION
     Paths recreated from the official PyrexxAI_logo.png mark:
       • Outer outline + inner gap/separator shapes → var(--logo-outline)
         / var(--logo-gap), theme-adaptive (dark charcoal outline on
         light backgrounds, soft muted lavender outline on dark).
       • Two ribbon shapes + top bridge → brand teal (#48C4C6) /
         purple (#8952A5), matching the real mark's colors.

  2. OUTLINE IS COMPLETELY STATIC — ONLY COLOR ROTATES
     The outline, inner-gap, separator, and top-center bridge paths
     use fixed solid fills and NEVER move or animate in any way.
     The two main ribbon paths share ONE linearGradient
     (id="pyrexx-flow") as their fill. That gradient's
     `gradientTransform` is rotated via SMIL <animateTransform>,
     pivoted on the mark's visual center.
       • 0°   → Phase 1: teal on the left arm / purple on the right
                arm — matches the upright logo exactly.
       • 180° → Phase 2: purple on the left arm / teal on the right
                arm — the color arrangement the logo would have if
                it were rotated 180° (upside down), achieved WITHOUT
                moving a single shape.
     The gradient sweeps continuously 0°→360°, so the mark cycles
     Phase 1 → Phase 2 → Phase 1 clockwise, indicating "loading".
     (The small top-center bridge piece keeps a static brand-teal
     fill — it carries its own scale() transform, which would put a
     shared gradient's coordinate space out of sync with the main
     ribbons; see inline comment at that path for detail.)

  3. THEME-ADAPTIVE BACKGROUND
     Background is var(--bg-base) — the same token the dashboard
     itself uses — so the loading screen is light-pastel in light
     mode and the new midnight-slate dark in dark mode. No separate
     "splash color" exists; the screen IS the dashboard background.

  4. NETWORK-AWARE BEHAVIOR
     On mount, the mark appears immediately in its static Phase 1
     state (no rotation yet). If the component is still mounted
     after a short grace period (350ms), the color-rotation
     "loading" indicator begins — i.e. genuinely slow loads get a
     visible loading cue, while fast/cached loads go straight from
     "show logo" to the zoom-in exit without ever animating.

  5. ZOOM TO CENTER ON EXIT
     Regardless of which state the mark was in, completion triggers
     a scale-up + fade "zoom into the dashboard" exit.

  6. REDUCED MOTION
     No gradient rotation, no glow pulse, no large exit zoom — the
     mark stays static at Phase 1 and simply fades.
*/

const SIZE = 156; // px — logo render size
const CX = 840;   // gradient/rotation pivot — center of the cropped viewBox
const CY = 700;

export default function LoadingAnimation() {
  const [reducedMotion, setReducedMotion] = useState(false);
  const [showColorRotation, setShowColorRotation] = useState(false);
  ...
  useEffect(() => {
    // Network-aware: only start the color-rotation "loading" cue if
    // we're still on screen after a brief grace period. Fast/cached
    // loads unmount before this fires and never animate at all.
    const t = setTimeout(() => setShowColorRotation(true), 350);
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 1 }}
      exit={{ opacity: 0, transition: { duration: reducedMotion ? 0.18 : 0.5, ease: [0.6, 0, 0.4, 1] } }}
      aria-live="polite"
      aria-label="Loading Pyrexx AI dashboard"
    >
      {/* Subtle ambient glow — brand colors, very low opacity, present
          in both themes as soft atmosphere rather than a visible
      ...
      {!reducedMotion && (
        <motion.div
          ...
          animate={{ scale: [0.94, 1.05, 0.94], opacity: [0.5, 0.9, 0.5] }}
      ...
      <motion.div
        ...
        exit={
          reducedMotion
            ...
            : { opacity: 0, scale: 11, transition: { duration: 0.6, ease: [0.6, 0, 0.4, 1] } }
        }
      >
        <svg
        ...
        >
          <defs>
            {/*
              Color-rotation gradient. Hard stops create two solid
              color zones (teal / purple) with a narrow transition
              band. gradientTransform rotates this whole field around
              the mark's center — the SHAPES never move, only which
              color falls on which shape changes.
            */}
            <linearGradient
              id="pyrexx-flow"
            ...
            >
              ...
              <stop offset="42%"  stopColor="#48C4C6" />
              <stop offset="58%"  stopColor="#8952A5" />
              <stop offset="100%" stopColor="#8952A5" />

              {showColorRotation && !reducedMotion && (
                <animateTransform
          ...
          {/* ── STATIC: outer outline silhouette ──────────────────── */}
          <path
          ...
          {/* ── ANIMATED: right-X ribbon — Phase 1 = purple ───────── */}
          <path
            fill="url(#pyrexx-flow)"
          ...
          {/* ── ANIMATED: left-X ribbon — Phase 1 = teal ──────────── */}
          <path
            fill="url(#pyrexx-flow)"
          ...
          {/*
            ── STATIC: top-center bridge — fixed teal ──────────────
            NOTE: This path carries its own `transform="scale(...)"`.
            SVG's `gradientUnits="userSpaceOnUse"` coordinates are
            evaluated in the coordinate space BEFORE that transform
            is applied — so sharing `#pyrexx-flow` here would put its
            rotation pivot (CX, CY) and color-stop band in a different
            visual location than for the untransformed ribbon paths,
            desyncing this piece from the main color rotation.
            The original logo also gives this small accent its own
            independent (mostly static-looking) gradient rather than
            tying it to the main ribbon coloring. Matching that intent,
            this piece stays a fixed brand-teal — its phase-1 color —
            while the two main ribbons do the color-rotation.
          */}
          <path
            fill="#48C4C6"
          ...
          {/* ── STATIC: white interlocking separators ─────────────── */}
          <path fill="var(--logo-gap)" d="M686.11 768.27c7.46 9.393 15.831 21.657 22.927 31.524l40.25 56.209c6.766 9.545 19.04 28.02 26.233 35.953-5.863 8.279-13.437 18.406-18.75 26.816-14.239 16.662-24.704 37.6-40.015 53.763-28.998 30.61-68.327 25.064-106.661 25.03l-88.078.09 106.109-149.079 35.554-50.014c6.737-9.427 15.109-21.713 22.43-30.293"/>
          <path fill="var(--logo-gap)" d="M526.073 373.738c33.701.805 67.77-.083 101.329.273 34.111.363 65.419-5.597 90.744 22.698 11.335 12.666 19.833 27.177 30.034 40.712 6.612 9.457 22.546 29.34 27.658 38.523-8.99 10.518-22.49 30.377-30.819 41.966l-58.617 81.704c-51.907-71.05-101.548-144.083-153.38-215.211-1.956-2.685-5.933-7.747-6.949-10.665"/>
          <path fill="var(--logo-gap)" d="M836.28 560.32a7490 7490 0 0 0 58.179 81.616c7.068 9.884 23.99 32.406 29.348 41.995-8.235 13.362-22.015 31.42-31.596 44.748l-56.846 79.001-87.677-123.46c4.831-8.819 23.963-34.552 30.495-43.507 19.142-26.236 38.419-54.875 58.098-80.393"/>
          <path fill="var(--logo-gap)" d="M896.418 475.575c12.203-15.717 23.797-33.341 35.44-49.508 6.429-8.926 13.344-19.865 20.722-27.874a81.5 81.5 0 0 1 18.733-15.215 62.9 62.9 0 0 1 23.766-8.271c10.803-1.389 143.615-2.314 150.84-.414L1042.332 519.77c-17.793 24.907-38.49 55.676-56.89 79.614-11.999-15.396-24.03-33.023-35.446-49.02z"/>
          <path fill="var(--logo-gap)" d="M985.449 768.603c4.013 3 25.026 33.64 29.487 40.018 44.25 63.267 90.196 125.546 134.27 188.906l-90.437-.011c-25.78.083-53.6 2.858-77.659-7.297-24.281-10.248-35.348-29.367-50.049-49.705-11.578-16.017-23.089-32.792-35.279-48.35 8.029-9.198 17.793-24.536 25.462-34.7 21.45-28.427 42.248-61.118 64.205-88.861"/>
        </svg>
      </motion.div>
