  ...
  return (
    <motion.section variants={itemV} className="card p-4 md:p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={onViewAll} className="flex items-center gap-1 text-[11px] font-semibold cursor-pointer transition-colors rounded-lg px-2 py-1" style={{ color: iconColor, background: iconBg }} onMouseOver={(e) => (e.currentTarget.style.opacity = "0.75")} onMouseOut={(e) => (e.currentTarget.style.opacity = "1")}>
      ...
      <main id={`${tabPanelId}-panel`} className="px-4 md:px-8 pt-5 pb-28">
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} variants={containerV} initial="hidden" animate="show" exit={{ opacity: 0, transition: { duration: 0.1 } }}>
            {activeTab === "dashboard" ? <DashboardPanel onSelectMeeting={setSelected} /> : activeTab === "analytics" ? <AnalyticsPanel /> : <ProfilePanel clinicId={clinicId} />}
          </motion.div>
        </AnimatePresence>
      </main>
